...
class NeuralBandit(_BasePolicyWithExploit):
    ...
    def predict(self, X, exploit = False, output_score = False):
        scores = torch.stack([n(X) for n in self.networks]).numpy()
        self.scores = scores
    ...
    def fit(self, X, a, r):
        ...
        scores_of_actions = torch.gather(scores, dim=1, index=a.unsqueeze(1).type(torch.cuda.LongTensor))
